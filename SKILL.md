...
For the complete reusable standard-humanoid route through Unreal IK retargeting, optional Blender normalization/offline baking, FBX/GLB delivery, and runtime QA, read [references/standard-humanoid-animation-pipeline.md](references/standard-humanoid-animation-pipeline.md). Select the canonical route from the authoritative source: prefer an audited UE-native Skeletal Mesh/Skeleton/material/animation package as the final exchange source when it already lives in Unreal and its licence permits non-UE export; keep the original VRM canonical for VRM assets; use Blender canonical when topology, UV, skin, rest-pose, unit or hierarchy repair is required. Never round-trip a stable UE-native asset through Blender merely for ceremony, and never allow undeclared engine-local edits to create divergent character versions. Apply the raw-import normalization gate before target mapping, classify whether an exported root became a bone or the Armature object, reject split centimeter/object-scale domains, and require Unreal's imported root reference scale to be exactly `[1,1,1]`. Reuse one versioned IK Rig/IK Retargeter set per target-skeleton version instead of generating a disposable set per action.

When a new Unreal project needs a self-contained UE-to-Godot glTF/GLB exporter, read [references/portable-unreal-godot-bridge.md](references/portable-unreal-godot-bridge.md). Prefer this bridge for an audited UE-native source; treat Unreal as the final exchange-conditioning stage and export the mesh, skin, rest skeleton, materials, textures and accepted target actions from one exact asset version. Install the bundled SpriteFlux-owned Editor plugin with `scripts/install_unreal_godot_bridge.ps1`; do not download or runtime-depend on the research-source plugin. Use `RenderOffScreen + SIMPLE` for headless material delivery, emit a hashed inventory report, and keep model/material and animation playback gates independent.

When the canonical character source is VRM, read [references/vrm-unreal-godot-pipeline.md](references/vrm-unreal-godot-pipeline.md). Keep the original VRM as the model/material/MToon/morph/SpringBone source for the runtime engine, use VRM4U in Unreal only for animation production and target retargeting, and hand the approved target action downstream as animation-only data. Treat keyed secondary-chain roots plus runtime SpringBone as a deliberate hybrid; do not bake the whole chain, discard the VRM physics metadata, or call spring-bone skinning a vertex-cloth simulation.
