...
- 后续受控的人类验收进一步选择 Mini 480P 单主体作为约 256 px 游戏角色的默认性价比方案。仅在动作包络、安全边距或源像素门失败时改变画幅、提高分辨率或增加主体；这项选择是已测工作流默认值，不是模型质量的普遍定律。
- 画幅不是固定的 4:3 合同。每个动作先计算 body/root/weapon union 与 20–30% 安全边距，再从供应商当时支持的固定比例中选择最贴合运动包络且浪费最少的比例。每次提交前仍需按当日官方价格和最接近的实测 token 锚点估算实际模型、分辨率、比例、时长、主体数和调用数。
